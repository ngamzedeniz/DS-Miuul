...
df.loc[df["sex"]=="male","cinsiyet"]="Erkek"
df[df["sex"]=="male"]["sex"]="Erkek"

df[df["sex"]=="male"]["new2"]=0
df["new2"]=0
df.head()
...
#Doldurmak istediğim değeri gördüm;
df["deck"].mode()

#Değerin doğrudan gelmediğini; series yapısı içinde geldiğini fark ettim;
type(df["deck"].mode())

#series yapısından doğrudan değeri çektiğim yapıyı oluşuturdum;
df["deck"].mode()[0]

#Boş değerleri mod değeri ile doldurdum;
...
df["age_flag2"] = df["age"].apply(age_30)

df.head(10)

df["age_flag"] = df["age"].apply(lambda x: 1 if x<30 else 0)
df[df["age_flag"]!=df["age_flag2"]]
...
#time kırılımında total_bill değer.....
#########################################
