...
df = pd.read_csv('Donem11/Hafta2/Kural_Tabanli_Siniflandirma/persona.csv')
df.head()
df.shape
df.info()
...
#Ucuncu cozum alternatifinde elde ettiğimiz sonucun yapısını inceleyelim;
type(df.pivot_table(values="PRICE",index="COUNTRY",aggfunc="count"))
df.pivot_table(values="PRICE",index="COUNTRY",aggfunc="count").reset_index()

#Birinci ve ikinci yolda elde ettiğimiz sonucun yapısını inceleyelim;
...
df.groupby(by=['COUNTRY']).agg({"PRICE": "mean"}).reset_index().sort_values("PRICE")
...
#############################################
df.groupby(["COUNTRY", 'SOURCE', "SEX", "AGE"]).agg({"PRICE": "mean"}).sort_values("AGE")
...
agg_df = df.groupby(by=["COUNTRY", 'SOURCE', "SEX", "AGE"]).agg({"PRICE": "mean"})
